...
import (
	...
	"github.com/tenderly/optimism/l2geth/rollup/util"
...
func NewEVM(ctx Context, statedb StateDB, chainConfig *params.ChainConfig, vmConfig Config) *EVM {
	...
	if chainConfig.IsEWASM(ctx.BlockNumber) {
		...
		//  extIntOpts := strings.Split(Config.EWASMInterpreter, ":")
		//  path := extIntOpts[0]
		//  options := []string{}
		//  if len(extIntOpts) > 1 {
		//    options = extIntOpts[1..]
		//  }
		//  evm.interpreters = append(evm.interpreters, NewEVMVCInterpreter(evm, Config, options))
		// } else {
		// 	evm.interpreters = append(evm.interpreters, NewEWASMInterpreter(evm, Config))
		// }
